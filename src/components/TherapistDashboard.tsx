...
const pillarNameById: Record<number, string> = Object.fromEntries(pillarData.map((p) => [p.id, p.name]));
  ...
  const clients = React.useMemo(() => {
    ...
    const items = Object.keys(map).map((id) => {
      ...
      return {
        ...
        flaggedCount: storage.getObservations(id).filter((o) => o.flagged).length,
      };
    });
          ...
          <Card className="p-4 bg-white border-slate-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-900 text-2xl">{clients.filter((c) => c.flaggedCount > 0).length}</p>
              </div>
              <Button
        ...
        <div className="grid grid-cols-1 gap-4">
            ...
            <Card key={c.id} className="p-6 bg-white border-slate-200 shadow-sm">
              <div className="flex items-center justify-between gap-6">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" className="hover:bg-red-50 hover:border-red-300 text-red-700" onClick={() => deleteOne(c.id, c.name)}>
          ...
          {filtered.length === 0 && (
            <Card className="p-12 bg-white border-slate-200 text-center shadow-sm">
              ...
              <p className="text-slate-500 text-sm">Ask a client to complete an assessment or reset filters.</p>
            </Card>
          )}
        </div>
      </div>
